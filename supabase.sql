...
-- Sincronizado con la BD real de Supabase (2026-02-22)
-- =============================================
-- NOTA: Este archivo es referencia/documentación.
-- El schema real está en Supabase Dashboard.
...
CREATE POLICY "Users pueden ver sus propias cancelaciones"
  ON public.cancellation_requests FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users pueden solicitar cancelación"
  ON public.cancellation_requests FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Usuarios autenticados pueden votar"
  ON public.review_helpful_votes FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Usuario puede eliminar su voto"
  ON public.review_helpful_votes FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Newsletters INSERT con validación"
  ON public.newsletters FOR INSERT TO public
